...
const getInitialSiteEndText = (): string => {
  try {
    ...
    return stored && stored.trim() !== '' ? stored : UI_TEXTS.settings.endText.default;
  } catch {
...
const getInitialEpilogueMainTitle = (): string => {
  try {
    ...
    return stored && stored.trim() !== '' ? stored : UI_TEXTS.settings.epilogueMessage.mainDefault;
  } catch {
...
const getInitialEpilogueSubTitle = (): string => {
  try {
    ...
    return stored && stored.trim() !== '' ? stored : UI_TEXTS.settings.epilogueMessage.subDefault;
  } catch {
  ...
  actions: {
    ...
    saveEpilogueMainTitle() {
      try {
        if (this.epilogueMainTitle && this.epilogueMainTitle.trim() !== '') {
          localStorage.setItem('hst_epilogue_main_title', this.epilogueMainTitle);
        } else {
          localStorage.removeItem('hst_epilogue_main_title');
        }
      } catch (error) {
        console.warn('Failed to save epilogue main title to localStorage:', error);
      }
    },
    ...
    saveEpilogueSubTitle() {
      try {
        ...
        console.warn('Failed to save epilogue sub title to localStorage:', error);
      }
    },
