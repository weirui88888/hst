<template>
  <section class="relative w-full select-none" :style="rootStyle">
    ...
    <div class="marquee-container" :key="renderKey">
      <!-- 第一行：从左往右 -->
...
const debugInfo = computed(() => ({
  ...
  extractedImagesLength: extractedImages.value.length,
  topImagesLength: topImages.value.length,
  bottomImagesLength: bottomImages.value.length,
}));
...
const rootStyle = computed(() => {
  ...
  return {
    ...
    marginLeft:
      (props.fullBleed ?? true) ? "calc(-1 * (100vw - 100%) / 2)" : undefined,
    marginRight:
      (props.fullBleed ?? true) ? "calc(-1 * (100vw - 100%) / 2)" : undefined,
    // 确保容器不会超出视口宽度
    maxWidth: "100vw",
...
watch(
  ...
  async (newLength, oldLength) => {
    ...
    if (newLength > 0 && oldLength === 0) {
      ...
      // 等待DOM更新完成
      await nextTick();
      debugLog.info("ImageMarquee DOM更新完成");
    }
  },
  { immediate: true },
);
