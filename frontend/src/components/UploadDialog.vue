          ...
          <div class="md:order-1 space-y-3">
            ...
            >
              ...
              >
                ...
                <span class="text-sm">{{ UI_TEXTS.upload.desktopUploadHint }}</span>
              </div>
            ...
            >
              ...
              >
                ...
                <span class="text-sm">{{ UI_TEXTS.upload.mobileUploadHint }}</span>
              </div>
              <button
                v-if="media.length > 0"
                type="button"
            ...
            <div class="flex items-center gap-3">
              <div class="flex-1 min-w-0" ref="tagsBox">
                ...
                >
                  ...
                  <span v-if="parsedTags.length === 0" class="text-sm text-neutral-400">
              ...
              <div class="ml-auto">
                <DatePicker
                  ...
                  :is24hr="true"
                  :popover="{ visibility: 'focus', placement: 'bottom', isFixed: true }"
                  color="orange"
                >
                  <template #default="{ inputValue, inputEvents }">
                    <input
                      ...
                      class="px-0 py-2 border-0 bg-transparent text-sm text-neutral-800 dark:text-neutral-200 outline-none w-[140px] text-right"
                    />
                  </template>
                </DatePicker>
              </div>
...
<script setup lang="ts">
  ...
  if (typeof window !== 'undefined') {
    document.addEventListener('mousedown', onGlobalMouseDown, { passive: true } as any);
  }
    ...
    for (const file of files) {
      ...
      (async () => {
        try {
          ...
          const uploadRes: any = await ossUploader.upload({ file, randomName: true });
          const ossUrl = uploadRes?.ossSourceUrl || '';
          // 替换 media 内对应项的 url 为真实 OSS URL，保持仅三个字段
          const idx = media.value.findIndex(
            (m: any) =>
              m.type === 'image' && (m.url === previewLocalUrl.value || m.url === mediaItem.url),
          );
          if (idx !== -1 && ossUrl) {
